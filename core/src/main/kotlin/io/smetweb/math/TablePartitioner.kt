...
class TablePartitioner<PK: Any>(
    ...
    override fun toString(): String =
        try {
            ...
            val keyStr = (if (n < 6) keys.toList() else listOf(keys[0], keys[1], null, keys[n - 2], keys[n - 1]))
                .joinToString(", ") { if (it == null) ".." else "@$it(${evaluator(root.limits!!, it)})" }
            buildString {

...
/**
 * [SchedulerService] extends [ClockService] with time controls and scheduled callback triggering,
 ...
 */
@Suppress("UNUSED")
interface SchedulerService: ClockService {
	...
	/** [ManagedTimer] provides the [Timer] scheduling API (as of Java 1.3) triggered by a managed [SchedulerService] */
	open class ManagedTimer(
			private val schedulerService: SchedulerService
	): Timer() {

		override fun schedule(task: TimerTask, delayMillis: Long) {
			val due = this.schedulerService.timeAfter(delayMillis)
			this.schedulerService.trigger( { task.run() }, { task.cancel() }, due)
		}

		override fun schedule(task: TimerTask, time: Date) {
			val due = this.schedulerService.timeOf(time)
			this.schedulerService.trigger({ task.run() }, { task.cancel() }, due)
		}
		...
		override fun scheduleAtFixedRate(task: TimerTask, delayMillis: Long, periodMillis: Long) {
			val due = this.schedulerService.timeAfter(delayMillis)
			this.schedulerService.trigger({ task.run() }, { task.cancel() }, due) {
						this.schedulerService.timeAfter(periodMillis)
					}
		}

		override fun scheduleAtFixedRate(task: TimerTask, firstTime: Date, periodMillis: Long) {
			val due = this.schedulerService.timeOf(firstTime)
			this.schedulerService.trigger({ task.run() }, { task.cancel() }, due) {
						this.schedulerService.timeAfter(periodMillis)
					}
		}

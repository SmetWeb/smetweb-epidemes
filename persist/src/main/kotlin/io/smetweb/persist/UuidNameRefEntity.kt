...
import io.smetweb.domain.ontology.ActorKind
...
data class UuidNameRefEntity(
	...
	constructor(
			...
			parentRefEntity: UuidNameRefEntity? = nameRef.getParentRef()?.getParentRef()?.let {
				nameRefFetcher(nameRef.getParentRef() as UuidNameRef)
			}
	): this(
			value = nameRef.get().toString(),
			contextRef = nameRef.getRootRef(),
